...
| `animation/` | Logo morph exports — mp4 + gif, for social. Not loaded by the app |
...
**`animation/` holds the social exports, and they will drift.** They are the
original 8s loop with the dark `#21262A` ground and the pre-token brand hexes
baked in — deliberately, because Threads and Reels need an opaque frame, not a
themed one. But that means they do not follow a palette change the way the
inline SVG does. Re-export them from the generator if the brand colours move.
They ship to Vercel as static files (~1.7MB) and no page requests them.
...
generated SVG. The mp4 and gif are **not** regenerated by it; those still come
out of a screen capture of the loop.
